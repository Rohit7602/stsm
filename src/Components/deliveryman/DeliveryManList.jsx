...
const DeliveryManList = () => {
  ...
  // Calculate order counts for all delivery men
  const calculateOrderCounts = () => {
    const counts = {};
    orders.forEach((order) => {
      if (order.assign_to) {
        counts[order.assign_to] = (counts[order.assign_to] || 0) + 1;
      }
    });
    return counts;
  };

  const orderCounts = calculateOrderCounts();
  ...
  return (
    <div className="main_panel_wrapper bg_light_grey w-100">
      {deletePopup && <div className="bg_black_overlay"></div>}
      <div className="w-100 px-sm-3 pb-4 mt-4 bg_body">
        <div className="d-flex flex-column flex-md-row align-items-center gap-2 gap-sm-0 justify-content-between">
          <div className="d-flex">
            <h1 className="fw-500 black fs-lg mb-0">List of Delivery Mans</h1>
            ...
            <div className="delivery_location_sidebar">
              {filterData.map((data) => (
                <>
                  <div className="d-flex align-items-center justify-content-between">
                    <div>
                      <p className="fs-sm fw-600 black m-0">
                        {data.basic_info.name}
                      </p>
                      <p className="fs-xs fw-400 black m-0 mt-1">
                        Service Area List
                      </p>
                    </div>
                    <img
